...
# ---------------------------------------------------------------------------
# ساختار منوی گروه‌بندی‌شده (v1.6.0)
# ---------------------------------------------------------------------------

def _last_inline_labels():
    """آخرین پیامی که دکمه inline داشته رو پیدا می‌کنه و برچسب‌هاش رو برمی‌گردونه."""
    import json
    for _, _, _, data in reversed(SENT_LOG):
        rm = data.get("reply_markup") if data else None
        if rm:
            kb = json.loads(rm)
...
def test_customer_main_menu_has_three_top_level_groups():
    register(110)
    labels = _last_inline_labels()
    assert labels == ["🧑‍⚖️ درخواست خدمت حقوقی", "📂 حساب و سفارش‌های من", "ℹ️ راهنما و پشتیبانی"]


def test_admin_main_menu_has_three_top_level_groups():
    SENT_LOG.clear()
    bot.handle_message({"chat": {"id": 999999}, "text": "/admin"})
    labels = _last_inline_labels()
    assert labels == ["🗂 سفارش‌ها و مشتری‌ها", "📢 بازاریابی و اطلاع‌رسانی", "📊 آمار، گزارش و راهنما"]


def test_back_button_label_differs_when_it_cancels_an_order():
    register(111)
    # ویرایش پروفایل: بازگشت ساده، نباید سفارشی لغو بشه
    cb(111, "menu_6")
    cb(111, "edit_phone")
    assert _last_inline_labels() == ["⬅️ بازگشت"]

    # وسط خرید: بازگشت باید سفارش نیمه‌کاره رو هم لغو کنه
    cb(111, "goback|profile|0")
    cb(111, "menu_services")
    cb(111, "menu_1")
    cb(111, "q_general")
    assert "🚫 انصراف و بازگشت" in _last_inline_labels()


def test_review_page_has_direct_cancel_button():
    register(112)
    cb(112, "menu_services")
    cb(112, "menu_1")
    cb(112, "q_general")
    bot.handle_message({"chat": {"id": 112}, "photo": [{"file_id": "R1"}]})
    bot.handle_message({"chat": {"id": 112}, "text": "سوال تستی"})
    assert "🚫 انصراف از این درخواست" in _last_inline_labels()
