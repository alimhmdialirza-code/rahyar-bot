...
def prompt_admin_search(chat_id):
    send_message(
        chat_id,
        "🔍 جستجوی مشتری\n\n"
        "برای جستجو، دستور زیر رو با نام، فامیل یا شماره تلفن مشتری بفرستید:\n\n"
        "/find علی\n"
        "/find 0912...",
    )
...
def show_admin_help(chat_id):
    send_message(
        ...
        "/admin — نمایش منوی مدیریت (یا دکمه «🛠 پنل مدیریت» پایین صفحه)\n"
        "/stats — نمایش آمار سفارش‌ها و کاربران\n"
        "/find نام‌یا‌تلفن — جستجوی مشتری\n"
        "/addcode کد درصد تعداد — ساخت کد تخفیف (مثال: /addcode WELCOME10 10 50)\n"
...
def handle_admin_message(chat_id, text, attachment_type=None, attachment_file_id=None):
    ...
    if text.startswith("/addcode"):
        ...
        if len(parts) != 4 or not parts[2].isdigit() or not parts[3].isdigit():
            send_message(
                ADMIN_CHAT_ID,
                "فرمت درست:\n/addcode کد درصد_تخفیف تعداد_مجاز\n\nمثال:\n/addcode WELCOME10 10 50",
            )
            return True
        _, code, percent, max_uses = parts
        db.create_discount_code(code.upper(), int(percent), int(max_uses))
        send_message(
            ADMIN_CHAT_ID,
            f"✅ کد تخفیف «{code.upper()}» ساخته شد.\n{percent}% تخفیف، حداکثر {max_uses} بار قابل استفاده.",
        )
        return True
