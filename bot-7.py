...
def handle_admin_callback(callback_data):
    ...
    if callback_data == "adm_menu_discounts":
        show_discount_menu(ADMIN_CHAT_ID)
        return
...
def show_admin_menu(chat_id):
    keyboard = make_keyboard([
        ...
        [("🎟 مدیریت کدهای تخفیف", "adm_menu_discounts")],
...
def start_discount_wizard(chat_id):
    ADMIN_WIZARD["flow"] = "discount_create"
    ...
    send_message(
        ...
        "🎟 ساخت کد تخفیف جدید\n\n"
        "مرحله ۱ از ۵\n"
        "یک اسم برای این کمپین انتخاب کنید (فقط برای شناسایی خودتون، مثلاً «تخفیف نوروز»):",
        cancel_wizard_keyboard(),
    )
...
def show_discount_confirmation(chat_id):
    ...
    desc_label = data.get("description") or "—"
    text = (
        "📋 خلاصه کد تخفیف جدید\n\n"
        f"نام: {data['name']}\n"
        f"توضیحات: {desc_label}\n"
        f"درصد تخفیف: {data['percent']}٪\n"
        f"سقف استفاده هر کد: {max_uses_label}\n"
        f"تعداد کد: {data['quantity']}\n\n"
        "تایید می‌کنید؟"
    )
...
def handle_discount_wizard_text(chat_id, text):
    ...
    if step == "name":
        if not text:
            ...
            return
        data["name"] = text
        ADMIN_WIZARD["step"] = "description"
        keyboard = make_keyboard([[("رد کردن (بدون توضیح)", "dcw_desc_skip")], [("❌ لغو", "dcw_cancel")]])
        send_message(chat_id, "مرحله ۲ از ۵\nتوضیح کوتاهی برای این کد بنویسید (اختیاری):", keyboard)

    elif step == "description":
        data["description"] = text
        ADMIN_WIZARD["step"] = "percent"
        send_message(chat_id, "مرحله ۳ از ۵\nچند درصد تخفیف؟", percent_keyboard())
        ...
        send_message(chat_id, "مرحله ۵ از ۵\nچند تا کد جداگانه نیاز دارید؟", quantity_keyboard())
        ...
        show_discount_confirmation(chat_id)
...
def handle_discount_wizard_callback(callback_data):
    ...
    if callback_data == "dcw_desc_skip":
        data["description"] = ""
        ADMIN_WIZARD["step"] = "percent"
        send_message(ADMIN_CHAT_ID, "مرحله ۳ از ۵\nچند درصد تخفیف؟", percent_keyboard())
        return
    ...
    if callback_data.startswith("dcw_percent_"):
        ...
        ADMIN_WIZARD["step"] = "maxuses"
        send_message(ADMIN_CHAT_ID, "مرحله ۴ از ۵\nهر کد چند بار قابل استفاده باشه؟", maxuses_keyboard())
        return
    ...
    if callback_data.startswith("dcw_maxuses_"):
        ...
        ADMIN_WIZARD["step"] = "quantity"
        send_message(ADMIN_CHAT_ID, "مرحله ۵ از ۵\nچند تا کد جداگانه نیاز دارید؟", quantity_keyboard())
        return
    ...
    if callback_data == "dcw_confirm":
        codes = db.create_discount_codes_batch(
            data["name"], data.get("description", ""), data["percent"], data["max_uses"], data["quantity"],
        )
        reset_admin_wizard()
        codes_text = "\n".join(f"🎟 {c}" for c in codes)
        send_message(
            ADMIN_CHAT_ID,
            f"✅ {len(codes)} کد تخفیف ساخته شد:\n\n{codes_text}\n\nاین کدها رو می‌تونید بین مشتری‌ها پخش کنید.",
        )
        return
...
def show_discount_menu(chat_id):
    keyboard = make_keyboard([
        [("➕ ساخت کد تخفیف جدید", "dcw_start")],
        [("📋 لیست کدهای فعال", "adm_dc_list")],
        [("🔙 بازگشت", "adm_menu_root")],
    ])
    send_message(chat_id, "🎟 مدیریت کدهای تخفیف", keyboard)


def show_discount_list(chat_id):
    codes = db.list_active_discount_codes()
    if not codes:
        send_message(chat_id, "هیچ کد تخفیف فعالی وجود نداره.")
        return
    for c in codes:
        ...
        text = (
            f"🎟 {c['code']} — {label}\n"
            f"{c['percent_off']}٪ تخفیف | استفاده‌شده: {c['used_count']} از {max_label}"
        )
...
def show_admin_help(chat_id):
    send_message(
        ...
        "🎟 مدیریت کدهای تخفیف — ساخت، مشاهده و غیرفعال‌سازی کد تخفیف\n\n"
...
def handle_admin_message(chat_id, text, attachment_type=None, attachment_file_id=None):
    ...
    if ADMIN_WIZARD["flow"] == "discount_create":
    ...
    if text.startswith("/addcode") or text == "/discount":
        send_message(
            ADMIN_CHAT_ID,
            "🎟 ساخت کد تخفیف حالا مرحله‌به‌مرحله‌ست، دیگه نیازی به تایپ دستور نیست.\n"
            "از منوی مدیریت (/admin) روی «🎟 مدیریت کدهای تخفیف» بزنید.",
        )
        return True
