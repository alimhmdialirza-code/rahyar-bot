...
def create_discount_code(code, percent_off, max_uses):
    with get_connection() as conn:
        conn.execute(
            "INSERT OR REPLACE INTO discount_codes (code, percent_off, max_uses, used_count) "
            "VALUES (?, ?, ?, COALESCE((SELECT used_count FROM discount_codes WHERE code = ?), 0))",
            (code, percent_off, max_uses, code),
        )
