...
def show_my_orders(chat_id):
    ...
    if not orders:
        send_message(
            chat_id, "📦 شما هنوز سفارشی ثبت نکردید.", make_keyboard([[("🔙 بازگشت به منوی اصلی", "back_main")]])
        )
        return
    for o in orders:
        status_fa = ORDER_STATUS_LABELS.get(o["status"], o["status"])
        price_part = f"\n💳 {o['price']} تومان" if o.get("price") else ""
        text = f"🆔 #{o['id']} | {o['order_type']}{price_part}\nوضعیت: {status_fa}"
        rows = []
        if o.get("final_reply_text") or o.get("final_reply_file_id"):
            rows.append([("📄 مشاهده پاسخ", f"view_reply_{o['id']}")])
        send_message(chat_id, text, make_keyboard(rows) if rows else None)
    send_message(chat_id, "برای بازگشت:", make_keyboard([[("🔙 بازگشت به منوی اصلی", "back_main")]]))
...
def show_payment_history(chat_id):
    ...
    keyboard = make_keyboard([[("🔙 بازگشت به منوی اصلی", "back_main")]])
    if not paid_orders:
        send_message(chat_id, "🧾 هنوز پرداختی ثبت نشده.", keyboard)
        return
    lines = []
    for o in paid_orders:
        status_fa = ORDER_STATUS_LABELS.get(o["status"], o["status"])
        date_part = (o.get("created_at") or "").split(" ")[0]
        lines.append(f"🆔 #{o['id']} | {o['order_type']}\n💳 {o['price']} تومان | 📅 {date_part}\nوضعیت: {status_fa}")
    send_message(chat_id, "🧾 تاریخچه پرداخت‌های شما:\n\n" + "\n\n".join(lines), keyboard)
...
def handle_admin_callback(callback_data):
    ...
    if callback_data == "adm_menu_version":
        send_message(ADMIN_CHAT_ID, f"ℹ️ نسخه فعلی ربات رهیار قانون: {BOT_VERSION}")
        return
...
def start_review(chat_id, base_step, text):
    ...
    keyboard = make_keyboard([
        [("✅ تایید و ارسال", f"review_confirm_{base_step}")],
        [("✏️ ویرایش متن", f"review_edit_{base_step}")],
    ])
    send_message(chat_id, f"📋 مرور نهایی\n\n{label}:\n\n{text}\n\nتایید می‌کنید؟", keyboard)
...
def main():
    ...
    send_message(ADMIN_CHAT_ID, f"✅ ربات رهیار قانون روشن شد. (نسخه {BOT_VERSION})", admin_persistent_keyboard())
    offset = 0
