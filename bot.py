...
def handle_admin_text(chat_id, text):
    """پیام‌های متنی ادمین: حالت پاسخ به مشتری، یا دستور دستی /reply."""
    if text == "/cancel_reply":
        ADMIN_PENDING_REPLY["order_id"] = None
        send_message(ADMIN_CHAT_ID, "لغو شد.")
        return True

    # حالت ۱: ادمین قبلاً روی «پاسخ به این مشتری» زده و منتظریم متن پاسخ رو بفرسته
    ...
    if pending_order_id:
        ...
        if not order:
            ...
            return True
        sent = send_message(order["chat_id"], f"📨 پاسخ از رهیار قانون:\n\n{text}")
        if sent:
    ...
    # حالت ۲: دستور دستی — /reply <chat_id> <متن پیام>
...
def handle_message(message):
    ...
    if chat_id == ADMIN_CHAT_ID and handle_admin_text(chat_id, text):
        return
